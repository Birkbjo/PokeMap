...
def fake_search_loop():
    while True:
        ...
        time.sleep(10)
#Generates locations for a given lat,lon tuple and steps.
def add_location_queue(latlon,steps):
    for step, step_location in enumerate(generate_location_steps(latlon, steps), 1):
...
def search_overseer_thread(args, new_location_queue, pause_bit):
    ...
    for i, account in enumerate(args.accounts[:args.thread_limit]):
